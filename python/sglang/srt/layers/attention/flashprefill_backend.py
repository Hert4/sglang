...
class FlashPrefillAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        if min_sparse_kv_len < 0:
            raise ValueError("min_sparse_kv_len must be non-negative")
        ...
        self.min_sparse_kv_len = min_sparse_kv_len
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if self.min_sparse_kv_len > 0 and not self.use_fa3_for_extend:
            # Chi phi dung chi muc phai tra o MOI do dai, nhung phan tiet kiem
            # lai ty le voi do dai KV. Duoi diem giao thi tra nhieu hon thu.
            # min_sparse_q_len khong the dien dat dieu nay: voi chunked prefill,
            # max_seq_len_q luon bang chunk size o moi chunk.
            if int(forward_batch.seq_lens_cpu.max().item()) <= self.min_sparse_kv_len:
                self.use_fa3_for_extend = True
    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        if k is not None and save_kv_cache:
            ...
            # main bo ForwardBatch.token_to_kv_pool; pool nam o backend
            # (self.token_to_kv_pool duoc gan tu model_runner trong __init__)
    ...
    def init_forward_metadata_out_graph(
        ...
        # Decode == FA3 -> uy quyen nguyen ven. Ban 0.5.10 goi ham nay la
        # init_forward_metadata_capture_cuda_graph voi 7 tham so roi; main doi
        # sang nhan thang ForwardBatch.
        self.decode_backend.init_forward_metadata_out_graph(
            forward_batch, in_capture=in_capture
        )

    def init_forward_metadata_in_graph(self, forward_batch: ForwardBatch):
        # Ban 0.5.10 goi la init_forward_metadata_replay_cuda_graph.
        self.decode_backend.init_forward_metadata_in_graph(forward_batch)

    def draft_extend_metadata_captured_in_graph(self) -> bool:
        return self.decode_backend.draft_extend_metadata_captured_in_graph()
